...
def visualizations():
    ...
    try:
        ...
        fig2.update_traces(marker_color='#4f46e5')
        fig2.update_xaxes(gridcolor='#f1f5f9')
        fig2.update_yaxes(gridcolor='#f1f5f9')
        graph2JSON = json.dumps(fig2, cls=plotly.utils.PlotlyJSONEncoder)
        
        # 3. Feature Contributions
        feature_cols = ['skill_match', 'interest_match', 'cgpa_match', 
                       'location_match', 'branch_match', 'experience_match']
        feature_means = allocations[feature_cols].mean()
        feature_labels = ['Skill Match', 'Interest Match', 'CGPA Match', 
                         'Location Match', 'Branch Match', 'Experience Match']
        ...
        labels = []
        values = []
        colors = ['#16a34a', '#2563eb', '#7c3aed', '#d97706', '#dc2626']
        ...
        fig4 = px.pie(
            values=values, 
            names=labels, 
            title='Allocation Quality Distribution',
...
def download(file_type):
    try:
        ...
        files = {
            'allocations': os.path.join('results', 'final_allocations.csv'),
            'unallocated': os.path.join('results', 'unallocated_students.csv'),
            'match_matrix': os.path.join('results', 'match_matrix.csv')
        }
        ...
        if not os.path.exists(filepath):
            ...
            if not os.path.exists(filepath):
                flash(f'File not found: {os.path.basename(filepath)}. Please run allocation first.', 'warning')
                return redirect(url_for('results'))
        
        # Get absolute path
        abs_filepath = os.path.abspath(filepath)
        
        print(f"Downloading file from: {abs_filepath}")  # Debug
        ...
        return send_file(
            ...
            mimetype='text/csv',
            as_attachment=True,
            download_name=os.path.basename(abs_filepath)
        )
...
def check_files():
    ...
    files_to_check = [
        'results/final_allocations.csv',
        'results/unallocated_students.csv', 
        'results/match_matrix.csv'
    ]
    ...
    # Also list what's actually in results folder
    results_dir = os.path.abspath('results')
    if os.path.exists(results_dir):
        results['files_in_results'] = os.listdir(results_dir)
